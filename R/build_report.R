...
build_report <- function(ind_scores, calibration_scores, EcoP, output_dir = NULL) {
  ...
  if (!dir.exists(fs::path(output_dir))) {
    cli::cli_abort("The selected output folder:{ vars output_dir} does not exist, please check and choose a folder that exists")
  }

  # check wetland_id is in the dataset
  if (!"wetland_id" %in% names(ind_scores)) {
    cli::cli_abort("There is no wetland id associated with the input data, please check and add this column for reporting")
  }


  #build site key to make sure the value is added to report
  site_key <- ind_scores |>
    dplyr::select( .data$site,  .data$wetland_id) |>
    unique()
 ...
 # add site key
  classed_df <- dplyr::left_join(classed_df, site_key, by = dplyr::join_by(site))
