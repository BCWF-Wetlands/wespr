---
title: "r4 Calculate wespr single site"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{r4 Calculate wespr single site}
...
Threshold values to define the low , medium or high class are determined by normalizing all the reference data for each value and then looking for natural breaks in the distribution (i.e. Jenks Breaks). These thresholds are then used as benchmarks and compared to our site of interest to determine which category it falls within. 

Note in some cases, a wetland site value may fall below or above data gathered for the calibration sites. In this case, a warning message is provided to the user.
...
If the ecoprovince does not yet have calibration data loaded, users should select the next best ecoprovince available. Ecoprovinces not yet calibrated include: Northern Boreal Mountains (NBM), and Coast and Mountains (CM)
...
Calibrated Ecoprovinces (as of April 2026) include the following: Central Interior (CI), Georgia Depression (GD), Southern Interior (SI), Southern Interior Mountains (SIM), Sub-Boreal Interior (SBI), and Boreal and Taiga Plains (BTP).

The output scores will include a column with a normalized rating (calibration_score) of L = lower M = moderate or H = higher (L,M,H)
...
Now you have a data table with all indicators, service_type (function or benefit), raw scores and normalized scores and ratings (L = lower, M = moderate, H = higher) and is referred to as "calibration_score" in the headings. This can be saved or exported as a .csv to be further explored. 

Users can also generate a summary report per site. This requires similar inputs as used for the assign_jenks_score() function. Users also need to define an output directory where the output file will be saved. In this example below, we use a folder called "temp"
