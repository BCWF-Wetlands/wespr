...
```{r plot map, echo = FALSE, fig.width=3.25, fig.height=3.25}
map1
...
fclass <-  classed_df|> 
   dplyr::filter(service_type == "f") |> 
   filter(!is.na(value)) |> 
   select(service_full_name, calibration_scores_summary)
...
This site ecosystem benefit scores included `r all_ranks_f |> filter(calibration_scores_summary == "H") |> pull(n)` High ranked benefits, `r all_ranks_f |> filter(calibration_scores_summary == "M") |> pull(n)` Medium ranked benefits and `r all_ranks_f |> filter(calibration_scores_summary == "L") |> pull(n)` Low ranked benefits 
...
### Which questions are driving the scores? 
...
# Ecoprovince Calibration Scores 
...
```{r, add calibration benefit map, echo = FALSE, fig.width=12, fig.height=10}
