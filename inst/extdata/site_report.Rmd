...
To provide appropriate baseline ecosystem scores, a 100 or more wetlands are assessed within each Ecoprovince and used as a reference data set. Scores of these reference sites are then categorized into low (L), medium (M) or high (H) classes using a jenks breaks method. This statistical method looks for natural breaks and clusters within the reference data set to identify threshold values for each class. We can used these ecoprovince specific thresholds to assign relative L, M, H for any site within the ecoprovince and thus allows sites from difference ecoprovinces to be compared across the Province. 
...
```{r plot map, echo = FALSE, fig.width=3.5, fig.height=3.25, fig.align = 'center'}
map1
