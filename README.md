...
benefit of the services provided by wetlands within British Columbia.
...
spatial layers, 2) preparing inuts form the field and desktop assessment
surveys, 3) calculates scores and ratings and outputs a report of
WESP-BC for a range of functions and attributes, 4) advanced functions
to upload and verify calibration data sets.
...
#### 2) Prepare input data for WESPR calculations

Once the user has answered the required field and desktop survey123
surveys (and output these as .csv files), users can choose from either a
simple or detailed workflow. For beginner R users, we recommend using
the [simplified analysis
pathways](https://bcwf-wetlands.github.io/wespr/articles/prepare-rawdata.html),
which condenses all the steps into a single function.
...
calculate the WESPR-BC scores and ratings.
...
#### 4) Calculate WESPR scores - multiple sites
...
provides advanced topics for those interested in understanding the
